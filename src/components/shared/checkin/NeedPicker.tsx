...
export function NeedPicker({ needs, value, onChange, error }: NeedPickerProps) {
  return (
    <fieldset aria-describedby={error ? "need-error" : undefined}>
      ...
      <div className="mx-auto mt-2 grid max-w-2xl grid-cols-2 gap-1.5">
        {needs.map((need) => {
          ...
          return (
            <button
              ...
              className={cn(
                "min-h-11 rounded-lg border px-2.5 py-1.5 text-sm font-medium leading-tight transition-[background-color,border-color,color,transform] sm:px-3 sm:text-base",
