...
export function DatePickerSheet({ open, onClose, onSave, initialDate, initialTime }: DatePickerSheetProps) {
  ...
  return (
    <>
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="date-title" className="alumia-elevated fixed inset-x-0 bottom-0 z-[70] max-h-[92svh] overflow-y-auto rounded-t-3xl p-5 sm:left-1/2 sm:bottom-8 sm:max-w-lg sm:-translate-x-1/2 sm:rounded-3xl sm:p-7">
        ...
        <div className="mt-6 flex justify-end gap-3 border-t border-border pt-5"><Button variant="ghost" onClick={() => { setTime(""); setSelectedDate(new Date()); }}>Limpar</Button><Button onClick={() => { onSave?.(selectedDate, time || null); onClose(); }}>Aplicar</Button></div>
      </section>
    </>
  );
}
