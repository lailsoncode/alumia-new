  ...
  return (
    <>
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="date-title" className="alumia-elevated fixed inset-x-0 bottom-0 z-[70] max-h-[92svh] overflow-y-auto rounded-t-3xl p-4 sm:left-1/2 sm:bottom-5 sm:max-w-lg sm:-translate-x-1/2 sm:rounded-3xl sm:p-5">
        ...
        <label htmlFor="task-time" className="mt-4 block text-sm font-semibold">Horário <span className="font-normal text-muted-foreground">(opcional)</span></label>
        ...
        <div className="mt-4 flex justify-end gap-3 border-t border-border pt-3"><Button variant="ghost" onClick={() => { setTime(""); setSelectedDate(new Date()); }}>Limpar</Button><Button onClick={() => { onSave?.(selectedDate, time || null); onClose(); }}>Aplicar</Button></div>
      </section>
    </>
  );
}
