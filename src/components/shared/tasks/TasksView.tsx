...
export function TasksView() {
  ...
  return (
    <section>
      ...
      {loading ? (
        <div className="mt-4 space-y-1" aria-label="Carregando tarefas">{[0, 1, 2].map((item) => <div key={item} className="h-20 animate-pulse rounded-2xl bg-muted" />)}</div>
      ) : tab === "hoje" ? (
        <div className="mt-4 grid gap-x-4 gap-y-1 xl:grid-cols-2">{sections.map((section) => (
