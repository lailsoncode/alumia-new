        ...
        <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(19rem,0.75fr)]">
          <div className="space-y-4">
            <Surface className="p-4 sm:p-5">
              ...
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-[width]" style={{ width: `${progress}%` }} role="progressbar" aria-label="Progresso de hidratação" aria-valuemin={0} aria-valuemax={goal} aria-valuenow={intake} /></div>
          ...
          <Surface variant="subtle" className="overflow-hidden">
            <img src={hydrationImage} alt="Personagem da Alumia segurando uma garrafa de água" className="aspect-video w-full object-cover lg:aspect-[4/3]" />
            <div className="p-4"><h2 className="text-lg font-semibold">No seu ritmo.</h2><p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">A referência diária é configurável e serve para acompanhar seus próprios hábitos. Ajuste o valor ao que foi orientado para você.</p></div>
          </Surface>
        </div>
      </div>

      {goalOpen && <><button type="button" className="fixed inset-0 z-40 cursor-default bg-foreground/25 backdrop-blur-sm" onClick={() => setGoalOpen(false)} aria-label="Fechar ajuste da referência" /><section role="dialog" aria-modal="true" aria-labelledby="goal-title" className="alumia-elevated fixed left-1/2 top-1/2 z-50 w-[calc(100%_-_2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-3xl p-5"><div className="flex items-start justify-between gap-3"><div><h2 id="goal-title" className="text-xl font-bold">Referência diária</h2><p className="mt-1 text-sm leading-relaxed text-muted-foreground">Defina um valor pessoal entre 250 e 10.000 ml.</p></div><Button variant="ghost" size="icon" onClick={() => setGoalOpen(false)} aria-label="Fechar"><AlumiaIcon icon={Cancel01Icon} size="sm" /></Button></div><form onSubmit={saveGoal} className="mt-4"><label htmlFor="goal-value" className="text-sm font-semibold">Quantidade em ml<input id="goal-value" type="number" min="250" max="10000" step="50" value={goalInput} onChange={(event) => setGoalInput(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-input bg-background px-4 text-base focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring/30" /></label><div className="mt-4 flex justify-end gap-2.5"><Button type="button" variant="ghost" onClick={() => setGoalOpen(false)}>Cancelar</Button><Button type="submit">Salvar referência</Button></div></form></section></>}
    </AppShell>
  );
}
